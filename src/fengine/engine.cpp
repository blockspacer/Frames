...
void Engine::init(std::string title, unsigned int width, unsigned int height) {
  ...
  m_scheduler.add(
      std::bind(&Engine::processPhysics, this, std::placeholders::_1), 120);
  m_scheduler.add(
      std::bind(&Engine::processRender, this, std::placeholders::_1), 60);
...
void Engine::setFramerateLimit(unsigned int framerate) {
  m_scheduler.setRate(2, framerate);
}
...
void Engine::processEvents() {
  ...
  while (m_window->pollEvent(event)) {
    ...
    if (event.type == sf::Event::Resized) {
      // m_window->setView(sf::View(sf::FloatRect(0, 0, event.size.width,
      // event.size.height)));
    }
